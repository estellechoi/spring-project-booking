...
public class ProductDaoSqls {
	...
	public static final String SELECT_DISPLAY_BY_ID = "select description, content, place_street, place_lot, place_name, tel, opening_hours, save_file_name from product, display_info, display_info_image, file_info where display_info.product_id = product.id and display_info_image.display_info_id = display_info.id and display_info_image.file_id = file_info.id and product.id = :id and display_info.id = :displayInfoId";
	public static final String SELECT_PRICE_BY_ID = "select id, price_type_name, price, discount_rate from product_price where product_id = :id";
}
